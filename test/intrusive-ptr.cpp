...
public:
    ...
    ~TestObject() override { ++destructed; }
...
public:
    ...
    explicit DerivedObject(int v = 0, int e = 0)
        : TestObject(v)
        , extra(e) {}
};
...
public:
    ...
    explicit ArenaTestObject(int v = 0)
        : value(v) {
        ++constructed;
    }

    ~ArenaTestObject() override { ++destructed; }
...
TEST_CASE("scoped_connection with intrusive_ptr", "[intrusive_ptr][signal]") {
    ...
    {
        sigslot::scoped_connection conn = sig.connect([&](int x) { received.push_back(x); });
...
TEST_CASE("multiple scoped_connections", "[intrusive_ptr][signal]") {
    ...
    for (int i = 0; i < 3; ++i) {
        conns.emplace_back(sig.connect([&, i](int x) { received.push_back(x * (i + 1)); }));
    }
