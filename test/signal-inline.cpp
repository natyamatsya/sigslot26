...
struct TrackedObject {
    ...
    int value;
    TrackedObject(int v = 0)
        : value(v) {
        ++constructions;
    }
    TrackedObject(const TrackedObject& o)
        : value(o.value) {
        ++constructions;
    }
    TrackedObject(TrackedObject&& o) noexcept
        : value(o.value) {
        ++constructions;
    }
...
TEST_CASE("signal_inline_seqlock concurrent connect and emit",
          "[signal_inline_seqlock][threaded]") {
    auto iteration = GENERATE_REPEAT();
    (void)iteration;
