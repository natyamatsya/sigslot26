...
struct plain_slot_storage {
    ...
    template<typename F>
    explicit plain_slot_storage(F&& f)
...
struct tracked_slot_storage {
    ...
    [[nodiscard]] bool is_valid() const noexcept { return !ptr.expired(); }
...
struct pmf_tracked_slot_storage {
    ...
    template<typename F, typename P>
    pmf_tracked_slot_storage(F&& f, P&& p)
        : pmf(std::forward<F>(f))
        , ptr(std::forward<P>(p)) {}
...
struct extended_slot_storage {
    ...
    template<typename F>
    explicit extended_slot_storage(F&& f)
        : func(std::forward<F>(f))
        , conn() {}
...
struct pmf_extended_slot_storage {
    ...
    template<typename F, typename P>
    pmf_extended_slot_storage(F&& f, P&& p)
        : pmf(std::forward<F>(f))
        , ptr(std::forward<P>(p))
        , conn() {}
...
#endif
    alignas(storage_align) std::byte storage_[storage_size] = {}; // 64 bytes - variant storage
...
public:
    ...
    slot_variant& operator=(slot_variant&& other) noexcept {
        if (this != &other) {
            ...
            blocked_.store(other.blocked_.load(std::memory_order_relaxed),
                           std::memory_order_relaxed);
            tag_ = other.tag_;
            destroy_ = other.destroy_;
    ...
    static slot_variant make_plain(Func&& f, Group gid) {
        ...
        slot.destroy_ = [](void* s) { std::launder(static_cast<storage_t*>(s))->~storage_t(); };
    ...
    static slot_variant make_tracked(Func&& f, WeakPtr&& ptr, Group gid) {
        ...
        // std::launder required after placement new
        slot.call_ = [](void* s, Args... args) -> bool {
            return std::launder(static_cast<storage_t*>(s))
                ->call_if_valid(std::forward<Args>(args)...);
        };
        slot.destroy_ = [](void* s) { std::launder(static_cast<storage_t*>(s))->~storage_t(); };
    ...
    bool disconnect() noexcept { return connected_.exchange(false, std::memory_order_acq_rel); }

    [[nodiscard]] bool blocked() const noexcept { return blocked_.load(std::memory_order_relaxed); }
    ...
    void set_index(std::size_t idx) noexcept { index_.store(idx, std::memory_order_relaxed); }

    [[nodiscard]] bool empty() const noexcept { return tag_ == slot_tag::empty; }
