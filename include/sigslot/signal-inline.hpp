...
public:
    ...
    void connect(Pmf&& pmf, Ptr&& ptr, group_id gid = group_id{}) {
        slots_.push_back(slot_type::make_pmf(std::forward<Pmf>(pmf), std::forward<Ptr>(ptr), gid));
    }
    ...
    void connect_tracked(Callable&& c, std::weak_ptr<Trackable> ptr, group_id gid = group_id{}) {
        slots_.push_back(slot_type::make_tracked(std::forward<Callable>(c), std::move(ptr), gid));
    }
...
public:
    ...
    void operator()(U&&... args) {
        ...
        if (slot_count_cache_.load(std::memory_order_acquire) == 0)
            return;
#endif
...
public:
    ...
    void connect(Callable&& c, group_id gid = group_id{}) {
        ...
        new_list->push_back(
            std::make_unique<slot_type>(slot_type::make_plain(std::forward<Callable>(c), gid)));
...
public:
    ...
    [[nodiscard]] T& operator[](std::size_t i) noexcept { return *std::launder(data() + i); }
    ...
    [[nodiscard]] bool empty() const noexcept { return size_.load(std::memory_order_acquire) == 0; }
...
public:
     ...
     */
    template<typename Callable>
    [[nodiscard]] std::expected<void, seqlock_error> connect(Callable&& c,
     ...
     */
    template<typename Pmf, typename Ptr>
    [[nodiscard]] std::expected<void, seqlock_error> connect(Pmf&& pmf, Ptr&& ptr,
                                                             group_id gid = group_id{}) {
        std::lock_guard lock(write_mutex_);
        if (slots_.full()) {
            return std::unexpected(seqlock_error::capacity_exceeded);
